...
export const PAYMENTS_MODE: PaymentsMode =
  (process.env.NEXT_PUBLIC_PAYMENTS_MODE as PaymentsMode | undefined) ?? "demo";
...
export function consumeCredit(): AccessRecord | null {
  ...
  const next = { ...current, creditsRemaining: current.creditsRemaining - 1 };
  writeAccess(next);
  return next;
}
...
export function canGenerate(): { ok: boolean; reason?: string } {
  ...
  if (access.plan === "free" && access.creditsRemaining === 0) {
    return { ok: false, reason: "Free pack used. Unlock more packs to continue." };
  }
...
export function paymentLinkFor(plan: "pro" | "lifetime"): string | undefined {
  if (plan === "lifetime") {
    return process.env.NEXT_PUBLIC_STRIPE_PAYMENT_LINK_LIFETIME;
  }
  return process.env.NEXT_PUBLIC_STRIPE_PAYMENT_LINK_PRO;
}
